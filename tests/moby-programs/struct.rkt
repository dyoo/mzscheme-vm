...
(let-values ([(a-struct-type
  ...
  (check-expect (struct-predicate-procedure? accessor) false)
  ...
  (check-expect (struct-accessor-procedure? mutator) false)
  (check-expect (struct-accessor-procedure? 24) false)
  (check-expect (struct-accessor-procedure? (lambda (x) true))
                                             false)

  (check-expect (struct-mutator-procedure? mutator) true)
