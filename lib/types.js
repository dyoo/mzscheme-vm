...
JsObject.prototype.toDomNode = function(cache) {
	return toDomNode(this.obj, cache);
};
	...
	newType.type.prototype.invokeEffect = function(aBigBang, k) {
		var schemeChangeWorld = new PrimProc('update-world', 1, false, false,
			function(worldUpdater) {
				...
				return new INTERNAL_PAUSE(function(restarter, caller) {
								aBigBang.changeWorld(function(w, k2) {
									caller(worldUpdater, [w], k2, function(e) { throw e; });
							  },
							  function() { restarter(VOID_VALUE); });
						});
			});

		var args = this._fields.slice(0, lastFieldIndex);
		args.unshift(schemeChangeWorld);
		return caller(impl, args, k);
	}
...
var ExnBreak = makeStructureType('exn:break', Exn, 1, 0, false,
		...
		// TODO: Put this back in when it doesn't create problems with breaks in world
//			helpers.check(cont, function(x) { return x instanceof ContinuationClosureValue; },
//				      name, 'continuation', 3);
...
var BigBangInfo = makeStructureType('bb-info', false, 2, 0, false,
			function(k, changeWorld, toplevelNode, name) {
				helpers.check(changeWorld, helpers.procArityContains(1), name, 'procedure (arity 1)', 1);
				helpers.check(toplevelNode, types.isJsObject, name, 'js-object', 2);

				k( new ValuesWrapper([changeWorld, toplevelNode]) );
			});
