...
var makeStructureType = function(name, initFieldCnt, autoFieldCnt, parentType) {
	...
	predicate: function(x) {
		if (x instanceof aStruct) {
			return true;
		}
		for (var i = 0; i < structType.children.length; i++) {
			if ( structType.children[i].predicate(x) ) {
				return true;
			}
		}
		return false;
    ...
    // if this has a parent type, push this onto the children
    if (parentType) {
	    parentType.children.push(structType);
    }
