...
var makeStructureType = function(theName, initFieldCnt, autoFieldCnt, parentType, autoV, guard) {
	...
	init: function(name, args) {
		// If there is a guard, we need to apply it appropriately
		var guardedArgs;
		if (guard) {
			var guardRes = guard.apply(null, args.concat([name]));
		...
		this._super(name, parentArgs);

		for (var i = 0; i < initFieldCnt; i++) {
			this._fields.push(guardedArgs[i+numParentArgs]);
		}
	...
	toString: function() { return '#<struct-type:' + theName + '>'; }
    };
};
...
var JsObject = function(name, parent, obj) {
	this.name = name;
	this.parent = parent;
	this.obj = obj;
};
...
JsObject.prototype.isEqual = function(other, aUnionFind) {
	return (this.parent === other.parent && this.obj === other.obj);
};
...
var GenericJsworldEffect = Class.extend({
	invokeEffect: function(k) {
		throw types.internalError('invokeEffect not implemented for generic effect', false);
	}
});

var makeJsworldEffect = function(f) {
	var Effect = GenericJsworldEffect.extend({invokeEffect: f});
	return new Effect();
};
...
var Posn = makeStructureType('posn', 2, 0, false, false, false);
var Color = makeStructureType('color', 3, 0, false, false, false);
var ArityAtLeast = makeStructureType('arity-at-least', 1, 0, false, false, false);
...
types.jsObject = function(name, parent, obj) { return new JsObject(name, parent, obj); };
...
var Exn = makeStructureType('exn', 2, 0, false, false,
		function(msg, contMarks, name) {
			helpers.check(msg, isString, name, 'string', 1);
			helpers.check(contMarks, types.isContinuationMarkSet, name, 'continuation mark set', 2);
			return new ValuesWrapper([msg, contMarks]);
		});
...
var ExnFail = makeStructureType('exn:fail', 0, 0, Exn, false, false);
types.exnFail = ExnFail.constructor;
types.isExnFail = ExnFail.predicate;

var ExnFailContract = makeStructureType('exn:fail:contract', 0, 0, ExnFail, false, false);
types.exnFailContract = ExnFailContract.constructor;
types.isExnFailContract = ExnFailContract.predicate;

var ExnFailContractArity = makeStructureType('exn:fail:contract:arity', 0, 0, ExnFailContract, false, false);
types.exnFailContractArity = ExnFailContract.constructor;
types.isExnFailContractArity = ExnFailContract.predicate;

var ExnFailContractVariable = makeStructureType('exn:fail:contract:variable', 1, 0, ExnFailContract, false, false);
...
var ExnFailContractDivisionByZero = makeStructureType('exn:fail:contract:division-by-zero', 0, 0, ExnFailContract, false, false);
...
types.makeJsworldEffect = makeJsworldEffect;
types.isJsworldEffect = function(x) { return x instanceof GenericJsworldEffect; };

var Effect = makeStructureType('effect', 1, 0, false, false,
		function(thunk, name) {
			helpers.check(thunk, helpers.procArityContains(0), name, 'procedure (arity 0)', 1);
			return thunk;
		});
types.effect = Effect.constructor;
types.isEffect = Effect.predicate;
types.effectThunk = function(e) { return Effect.accessor(e, 0); };
