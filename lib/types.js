...
var UnionFind = function() {
	...
	// to its parent.
	this.parentMap = types.makeLowLevelEqHash();
}
...
//		}
		...
		throw new Error('Str.make expects and array of 1-character strings or a string;' +
				' given ' + s.toString());
	}
}
...
Str.prototype.toDisplayString = Str.prototype.toString;

Str.prototype.copy = function() {
	return Str.makeInstance(chars.slice(0));
}
...
Str.prototype.substring = function(start, end) {
	...
	return Str.makeInstance( chars.slice(start, end) );
}
...
Str.prototype.isEqual = function(other, aUnionFind) {
	if (other === null || other === undefined || (! other instanceof Str)) {
		return false;
	}
...
Str.prototype.toUpperCase = function() {
	return Str.fromString( chars.join("").toUpperCase() );
}

Str.prototype.toLowerCase = function() {
	return Str.fromString( chars.join("").toLowerCase() );
}
