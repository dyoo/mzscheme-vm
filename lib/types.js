...
Effect.isEffectType = true;

var makeEffectType = function(name, parentType, initFieldCnt, impl, guard) {
	if ( !parentType ) {
		parentType = Effect;
	}
	
	var newType = makeStructureType(name, parentType, initFieldCnt, 0, false, guard);
	newType.type.prototype.invokeEffect = function(k) {
		impl.apply(null, [k].concat(this._fields));
	}
	newType.isEffectType = true;
...
types.isEffectType = function(x) { return (x instanceof StructType && x.isEffectType); };


types.isEffect = Effect.predicate;

var EffectDoNothing = makeEffectType('effect:do-nothing', false, 0, function(k) { k(); }, false);
