...
var makeEffectType = function(name, parentType, initFieldCnt, impl, newHandlerIndices, wrapHandler, guard) {
	if ( !parentType ) {
		parentType = Effect;
	}
	...
	newType.type.prototype.invokeEffect = function(k, changeWorld) {
		...
		for (var i = 0; i < handlerIndices.length; i++) {
			args[ handlerIndices[i] ] = wrapHandler(args[ handlerIndices[i] ], changeWorld);
		}
		args.unshift(k);
