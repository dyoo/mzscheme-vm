...
var makeEffectType = function(name, parentType, initFieldCnt, impl, guard, caller) {
	if ( !parentType ) {
		parentType = Effect;
	}
...
//			helpers.map(newHandlerIndices, function(x) { return x + newType.firstField; }) );
	newType.type.prototype.invokeEffect = function(k, changeWorld) {
		var schemeChangeWorld = new PrimProc('update-world', 1, false, false,
				function(world-updater) {
					helpers.check(world-updater, helpers.procArityContains(1),
						      'update-world', 'procedure (arity 1)', 1);
					
					changeWorld(function(w, k2) { caller(world-updater, [w], k2); },
						    function() {});
					return types.VOID;
				});
