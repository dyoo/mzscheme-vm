...
var WorldEffect = Class.extend({
	invokeEffect: function(k) {
		throw types.internalError('invokeEffect not implemented for generic effect', false);
	}
});

var makeWorldEffect = function(f) {
	var Effect = WorldEffect.extend({invokeEffect: f});
	return new Effect();
};


var WrappedWorldEffect = function(thunk) {
	this.thunk = thunk;
}
...
types.makeWorldEffect = makeWorldEffect;
types.isWorldEffect = function(x) { return x instanceof WorldEffect; };

types.wrappedWorldEffect = function(thunk) { return new WrappedWorldEffect(thunk); };
types.isWrappedWorldEffect = function(x) { return x instanceof WrappedWorldEffect; };
