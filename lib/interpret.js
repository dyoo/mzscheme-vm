...
var FRAME_PUSH = function(state) {
    ...
    state.pushValue(new UndefinedValue());
};
...
var CALL = function(n) {
	...
	if (procValue instanceof PrimitiveValue) {
	    throw new Error("Primitive " + procValue.name + " not implemented!");
	} else if (procValue instanceof ClosureValue) {
	    // At this point, arguments on are on the stack.
	    // Handle rest arguments
	    if (procValue.isRest) {
		sys.print("isRest has not been implemented!");
	    }
	    state.pushControl(procValue.body);
	} else {
...
State.prototype.evaluatePrimval = function(aPrimval) {
    return new PrimitiveValue(aPrimval.value.val);
};
...
State.prototype.executeApplication = function(anApp) {
    ...
    if (rands.length !== 0) {
	this.pushControl(SWAP(rands.length-1));
    }
    ...
    if (rands.length !== 0) {
	this.pushControl(SET(rands.length-1));
    }
...
var PrimitiveValue = function(name) {
    this.name = name;
};
