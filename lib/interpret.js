...
CaseLambdaComputeControl.prototype.invoke = function(state) {
	...
	state.setValueRegister(new types.CaseLambdaValue(this.name, clauses));
    } else {
...
var makeClosureValsFromMap = function(state, closureMap, closureTypes) {
	...
	var val, type;
	val = state.peekn(closureMap[i].toFixnum());
	type = closureTypes[i];
...
State.prototype.executeAssign = function(anAssign) {
    ...
    cmds.push(this.SET_TOPLEVEL(anAssign.id.depth.toFixnum(),
				anAssign.id.pos.toFixnum(),
				anAssign['undef-ok?']));
    this.pushManyControls(cmds);
};
...
State.prototype.executeVarref = function(aVarref) {
    var depth, pos;
    depth = aVarref.toplevel.depth.toFixnum();
    pos = aVarref.toplevel.pos.toFixnum();
