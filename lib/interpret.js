...
var loadMod = function(state, modCode) {
    return modCode;
    //return new ModCommand(modCode);
};

// var ModCommand = function(modCode) {
//     this.prefix = modCode.prefix;
//     this.body = map(loadCode, modCode.body);
// };

// ModCommand.prototype.invoke = function(state) {
//      processPrefix(state, this.prefix);
//      var cmds = [];
//      // FIXME: why is this copying?
...
var processIndirects = function(state, indirects) {
	...
	state.heap[anIndirect.id] = evaluateLam(state, 
						anIndirect.lam);
    }
};
...
var executeApplication = function(state, anApp) {
    ...
    // CALL will be responsible for popping off the 
...
var executeVarref = function(state, aVarref) {
    ...
    assert.ok(state.vstack[state.vstack.length - 1 - depth] instanceof types.PrefixValue);
