...
var run = function(aState, onSuccessK, onFailK, callSite) {
	...
	if (aState.breakRequested) {
	    ...
	    setTimeout(function() { aState.pausedForGas = false;
			    	    run(aState, onSuccessK, onFailK, callSite); },
		       0);
	} else {
	    onSuccessK(aState.v);
	    return;
	...
	if (e instanceof control.PauseException) {
		...
		var onSuccess = function(v, callSite) {
			aState.restore(stateValues);
			aState.v = v;
			run(aState, onSuccessK, onFailK, callSite);
		};
		var onFail = function(e2) {
			aState.restore(stateValues);
			onFailK( completeError(e2) );
		};
		var onCall = makeOnCall(aState);
	    	e.onPause(onCall, onSuccess, onFail);
	}
	else {
		onFailK( completeError(e) );
	}
    }
};
...
var call = function(state, operator, operands, k, onFail, callSite) {
    if ( state.pausedForGas ) {
	    setTimeout(function() { call(state, operator, operands, k, onFail, callSite); }, 1);
	    return;
    }
