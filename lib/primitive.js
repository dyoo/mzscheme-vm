...
var makeCaller = function(aState) {
	...
	return function(operator, operands, k, callSite) {
		interpret.call(aState, operator, operands, k, onFail, callSite);
	}
};
...
var schemeProcToJs = function(aState, schemeProc) {
	var caller = function(operator, operands, k) {
		interpret.call(aState, operator, operands, k, function(e) { throw e; });
	}
	return function(k) {
		var args = [];
    ...
    new PrimProc('prim-js->scheme',
		 ...
		 function(x) {
		 	...
		 	if ( typeof(x.obj) === 'number' ) {
				...
				// FIXME: This is a hack that creates problems with conversion of vectors to arrays and back
    ...
    new PrimProc('procedure->void-js-fun',
		 ...
		 function(aState, proc) {
			...
			aState.v = types.jsObject(proc.name + ' (void)', function() {
				var args = helpers.map(helpers.wrapJsObject, arguments);
				caller(proc, args, function() {}, 'proc->js: ' + proc.name);
			});
		 });
