...
var isCompoundEffect = function(x) {
	return ( types.isWrappedWorldEffect(x) || isListOf(x, isCompoundEffect) );
};
...
PRIMITIVES['make-world-config'] =
    new PrimProc('make-world-config',
		 ...
		 function(startup, shutdown, handlers) {
			...
			check(shutdown, funArityContains(1), 'make-world-config', 'procedure (arity 1)', 2);
			arrayEach(handlers, function(x, i) { check(x, isFunction, 'make-world-config', 'handler', i+3); });

			if ( !funArityContains(handlers.length)(startup) ) {
...
PRIMITIVES['make-world-effect'] =
    new PrimProc('make-world-effect',
		 1,
		 false, false,
		 function(thunk) {
		 	check(thunk, funArityContains(0), 'make-world-effect', 'procedure (arity 0)', 1);
			return types.wrappedWorldEffect(thunk);
		 });
...
PRIMITIVES['js-call'] =
    new PrimProc('js-call',
		 ...
		 function(objF, initArgs) {
			...
			return helpers.wrapJsObject( objF.obj.apply(null, args) );
		 });
