...
var isAssocList = function(x) {
	return isListOf(x, function(y) { return isPair(y) &&
						isPair(y.rest()) &&
						isEmpty(y.rest().rest()); });
};
...
//PRIMITIVES['coerce-world-handler'] =
//    new PrimProc('coerce-world-handler',
//		 1,
//		 false, false,
//		 function(handler) {
//		 	check(handler, isFunction, 'coerce-world-handler', 'procedure', 1);
//			return types.coercedWorldHandler(handler);
//		 });


PRIMITIVES['make-effect'] = new PrimProc('make-effect', 1, false, false, types.effect);
//		 function(thunk) {
//		 	check(thunk, procArityContains(0), 'make-world-effect', 'procedure (arity 0)', 1);
//			return types.effect(thunk);
//		 });
...
PRIMITIVES['prim-js->scheme'] =
    new PrimProc('prim-js->scheme',
		 ...
		 false, false,
		 function(x) {
		 	check(x, function(y) { return types.isJsObject(y) &&
						      ( typeof(y.obj) == 'number' ||
							typeof(y.obj) == 'string' ||
...
PRIMITIVES['get-js-object'] =
    new PrimProc('get-js-object',
		 ...
		 function(root, firstSelector, selectors) {
		 	...
		 	check(root, types.isJsObject, 'get-js-object', 'js-object', 1);
...
PRIMITIVES['get-js-array-field'] =
    new PrimProc('get-js-array-field',
		 2,
		 false, false,
		 function(objArray, index) {
		 	check(objArray, function(x) { return types.isJsObject(x) && x.obj instanceof Array; },
			      'get-js-array-field', 'javascript array', 1);
			check(index, isNatural, 'get-js-array-field', 'exact non-negative integer', 2);

			var i = jsnums.toFixnum(index);
			return types.jsObject(objArray.name+'['+i+']', objArray.obj[i]);
		 });
...
PRIMITIVES['js-set-field!'] =
    new PrimProc('js-set-field!',
		 ...
		 function(obj, field, val) {
		 	check(obj, function(x) { return types.isJsObject(x) && typeof(x.obj) == 'object'; },
...
PRIMITIVES['js-set-array-field!'] =
    new PrimProc('js-set-array-field!',
		 3,
		 false, false,
		 function(objArray, index, val) {
		 	check(objArray, function(x) { return types.isJsObject(x) && x.obj instanceof Array; },
			      'js-set-array-field!', 'javascript array', 1);
			check(index, isNatural, 'js-set-array-field!', 'exact non-negative integer', 2);

			var i = jsnus.toFixnum(index);
			objArray.obj[i] = (types.isJsObject(val) ? val.obj : val);
			return types.VOID;
		 });
...
PRIMITIVES['js-call'] =
    new PrimProc('js-call',
		 ...
		 function(fun, parent, initArgs) {
		 	check(fun, function(x) { return types.isJsObject(x) && typeof(x.obj) == 'function'; },
			      'js-call', 'javascript function', 1);
