...
PRIMITIVES['scheme->primitive-js'] =
    new PrimProc('scheme->primitive-js',
		 1,
		 false, false,
		 function(x) {
		 	check(x, function(y) { return ( isReal(y) ||
							...
							isBoolean(y) ); },
//							isFunction(y)); },
			      'scheme->primitive-js', 'real number, string, symbol, char, or boolean', 1);
...
//				}
//			}
			return wrapJsObject(returnVal);
		 });


PRIMITIVES['primitive-js->scheme'] =
    new PrimProc('primitive-js->scheme',
		 1,
		 false, false,
		 function(x) {
		 	check(x, function(y) { return types.isJsObject(y) &&
			      ...
			      'primitive-js->scheme', 'javascript number, string, boolean, or function', 1);
...
PRIMITIVES['js-call'] =
    new PrimProc('js-call',
		 ...
		 function(objF, initArgs) {
			...
			return wrapJsObject( objF.obj.apply(null, args) );
		 });
