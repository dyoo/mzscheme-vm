...
var checkFormatStr = "~s: expects type <~s> as ~s argument, given: ~s~s";
...
var check = function(x, f, functionName, typeName, position, otherArgs) {
	...
	if ( !f(x) ) {
		...
		var msg = helpers.format(checkFormatStr, [functionName,
					 		  typeName,
					 		  nthStr(position),
...
PRIMITIVES['js-big-bang'] =
    new PrimProc('js-big-bang',
		 ...
		 function(state, initW, handlers) {
		 	arrayEach(handlers,
				function(x, i) {
					check(x, function(y) { return isWorldConfigOption(y); },
					      'js-big-bang', 'handler', i+2);
				});
			var unwrappedConfigs = 
			arrayMap(function(x) { return function(config) { return x.configure(config); } },
				 handlers);
			console.log(unwrappedConfigs);
...
// arrayMap: (X -> Y) (arrayof X) -> (arrayof Y)
var arrayMap = function(f, arr) {
    var results = [];
    for (var i = 0; i < arr.length; i++) {
