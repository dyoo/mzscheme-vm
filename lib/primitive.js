...
var quicksort = function(functionName) {
	return function(initList, comp) {
		...
		var quicksortHelp = function(lst) {
			...
			return CALL(filter, [function(x) {return comp(lst.first(), x)}, lst.rest()],
				function(half1) {
...
var isFunction = function(x) {
	...
	// TODO Do we want ContinuationClosureValue too?
}
    ...
    new PrimProc('procedure-arity',
		 ...
		 function(proc) {
			...
			var singleCaseArity = function(aCase) {
				if (aCase.isRest) {
					return types.makeArityAtLeast(aCase.numParams);
				}
				else {
			...
			if ( proc instanceof PrimProc ||
				...
				for (var i = 0; i < proc.cases.length; i++) {
					types.cons(singleCaseArity(proc.cases[i]), ret);
				}
				return ret.reverse();
			}
				...
				for (var i = 0; i < proc.closures.length; i++) {
					types.cons( singleCaseArity(proc.closures[i]), ret );
				}
				return ret.reverse();
			}
			else {
...
PRIMITIVES['compose'] =
    new PrimProc('compose',
		 ...
		 function(procs) {
			...
			var composeHelp = function(x, fLst) {
				if ( fLst.isEmpty() ) {
					return x;
				}

				return CALL(funciton(args) {
						return callWithValues(fList.first(), args);
					    },
					    [x],
					    function(result) {
						return composeHelp(result, fLst.rest());
					    });
			}
			return function(x) { return composeHelp(x, funList); };
		 });
*/
...
PRIMITIVES['make-hash'] =
    new CasePrimitive(
	[new PrimProc('make-hash', 0, false, false, function() { return types.makeHash(types.EMPTY); }),
	 new PrimProc('make-hash',
		      1,
		      false, false,
		      function(lst) {
			  checkListOf(lst, isPair, 'make-hash', 'list of pairs', 1);
			  return types.makeHash(lst);
		      }) ]);

PRIMITIVES['make-hasheq'] =
    new CasePrimitive(
	[new PrimProc('make-hasheq', 0, false, false, function() { return types.makeHashEq(types.EMPTY); }),
	 new PrimProc('make-hasheq',
		      1,
		      false, false,
		      function(lst) {
			  checkListOf(lst, isPair, 'make-hasheq', 'list of pairs', 1);
			  return types.makeHashEq(lst);
		      }) ]);
