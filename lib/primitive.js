...
var jsp = function(userArgs, attribList) {
	...
	var node = helpers.wrapJsObject( jsworld.Jsworld.p(attribs) );
...
PRIMITIVES['js-p'] = makeOptionPrimitive('js-p', 0, [types.EMPTY], false, jsp);


var jsdiv = function(userArgs, attribList) {
	checkListOf(attribList, isAssocList, 'js-div', '(listof X Y)', 1, userArgs);

	var attribs = assocListToHash(attribList);
	var node = helpers.wrapJsObject( jsworld.Jsworld.div(attribs) );
...
PRIMITIVES['js-div'] = makeOptionPrimitive('js-div', 0, [types.EMPTY], false, jsdiv);


var jsButtonBang = function(funName) {
	return function(userArgs, worldUpdateF, effectF, attribList) {
		...
		var attribs = assocListToHash(attribList);
		var node = helpers.wrapJsObject( jsworld.Jsworld.buttonBang(worldUpdateF, effectF, attribs) );
...
var jsButton = function(userArgs, updateWorldF, attribList) {
	var noneF = new types.PrimProc('', 1, false, false, function(w) { return types.EMPTY; });
	return jsButtonBang('js-button')(userArgs, updateWorldF, noneF, attribList);
};
PRIMITIVES['js-button'] = makeOptionPrimitive('js-button', 1, [types.EMPTY], false, jsButton);

PRIMITIVES['js-button!'] = makeOptionPrimitive('js-button!', 2, [types.EMPTY], false, jsButtonBang('js-button!'));


var jsInput = function(userArgs, type, updateF, attribList) {
	...
	var attribs = assocListToHash(attribList);
	var node = helpers.wrapJsObject( jsworld.Jsworld.input(type.toString(), updateF, attribs) );
...
PRIMITIVES['js-input'] = makeOptionPrimitive('js-input', 2, [types.EMPTY], false, jsInput);



var jsImg = function(userArgs, src, attribList) {
	check(src, isString, "js-img", "string", 1, userArgs);
	checkListOf(attribList, isAssocList, 'js-img', '(listof X Y)', 2, userArgs);

	var attribs = assocListToHash(attribList);
	var node = helpers.wrapJsObject( jsworld.Jsworld.img(src, attribs) );
...
PRIMITIVES['js-img'] = makeOptionPrimitive('js-img', 1, [types.EMPTY], false, jsImg);
...
PRIMITIVES['js-text'] =
    new PrimProc('js-text',
		 ...
		 function(s) {
			...
			var node = helpers.wrapJsObject( jsworld.Jsworld.text(s, []) );
...
var jsSelect = function(userArgs, optionList, updateF, attribList) {
	...
	var node = helpers.wrapJsObject( jsworld.Jsworld.select(options, updateF, attribs) );
...
PRIMITIVES['js-select'] = makeOptionPrimitive('js-select', 2, [types.EMPTY], false, jsSelect);
...
PRIMITIVES['js-big-bang'] =
    new PrimProc('js-big-bang',
		 1,
		 true, true,
		 function(state, initW, configs) {
		 	arrayEach(configs,
				  function(x, i) { check(x,
							 function(y) {
								return (types.isWorldConfig(y) ||
...
//		 	check(startup, isFunction, 'make-world-config', 'procedure', 1, arguments);
//			check(shutdown, procArityContains(1), 'make-world-config', 'procedure (arity 1)', 2, arguments);
...
////			}
//
//			return types.worldConfig(startup, shutdown);
//		 });
