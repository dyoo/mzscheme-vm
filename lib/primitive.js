...
var quicksort = function(functionName) {
	return function(initList, comp) {
		...
		check(comp, procArityContains(2), functionName, 'procedure (arity 2)', 2);
...
var getMakeStructTypeReturns = function(aStructType) {
	...
	return new types.ValuesWrapper(
		[aStructType,
		 (new StructConstructorProc('make-'+name,
					    aStructType.numberOfArgs,
					    false,
					    false,
					    aStructType.constructor)),
		 (new StructPredicateProc(name+'?', 1, false, false, aStructType.predicate)),
		 (new StructAccessorProc(name+'-ref',
					 2,
					 false,
					 false,
					 function(x, i) {
		 ...
		 (new StructMutatorProc(name+'-set!',
					3,
					false,
					false,
					function(x, i, v) {
...
PRIMITIVES['make-struct-type'] =
	makeOptionPrimitive(
	    ...
	    function(aState,
		     ...
		     guard) {
		check(name, isSymbol, 'make-struct-type', 'symbol', 1);
		check(superType, function(x) { return x === false || types.isStructType(x); },
		      'make-struct-type', 'struct-type or #f', 2);
		check(initFieldCnt, isNatural, 'make-struct-type', 'exact non-negative integer', 3);
		check(autoFieldCnt, isNatural, 'make-struct-type', 'exact non-negative integer', 4);
		// TODO: check props
		// TODO: check inspector
		// TODO: check procSpect
		checkListOf(immutables, isNatural, 'make-struct-type', 'exact non-negative integer', 9);
		check(guard, function(x) { return x === false || isFunction(x); },
		      'make-struct-type', 'procedure or #f', 10);
	...
	makeOptionPrimitive(
	    ...
	    function(accessor, fieldPos, fieldName) {
	    	check(accessor, function(x) { return x instanceof StructAccessorProc && x.numParams > 1; },
		      'make-struct-field-accessor', 'accessor procedure that requires a field index', 1);
		check(fieldPos, isNatural, 'make-struct-field-accessor', 'exact non-negative integer', 2);
		check(fieldName, function(x) { return x === false || isSymbol(x); },
		      'make-struct-field-accessor', 'symbol or #f', 3);
	...
	makeOptionPrimitive(
	    ...
	    function(mutator, fieldPos, fieldName) {
	    	check(mutator, function(x) { return x instanceof StructMutatorProc && x.numParams > 1; },
		      'make-struct-field-mutator', 'mutator procedure that requires a field index', 1);
		check(fieldPos, isNatural, 'make-struct-field-mutator', 'exact non-negative integer', 2);
		check(fieldName, function(x) { return x === false || isSymbol(x); },
		      'make-struct-field-mutator', 'symbol or #f', 3);
	    	var fixnumPos = jsnums.toFixnum(fieldPos);
...
PRIMITIVES['call-with-values'] =
    new PrimProc('call-with-values',
		 ...
		 false, false,
		 function(g, r) {
		 	check(g, procArityContains(0), 'call-with-values', 'procedure (arity 0)', 1);
...
PRIMITIVES['js-object?'] = new PrimProc('js-object?', 1, false, false, types.isJsObject);
...
PRIMITIVES['filter'] =
    new PrimProc('filter',
		 ...
		 false, false,
		 function(f, lst) {
		 	check(f, procArityContains(1), 'filter', 'procedure (arity 1)', 1);
...
var jsSelect = function(optionList, updateF, attribList) {
	...
	node.toDisplayedString = node.toWrittenString;
	node.toDomNode = function(cache) { return node; };
	return node;//helpers.wrapJsObject(node);
};
...
PRIMITIVES['js-big-bang'] =
    new PrimProc('js-big-bang',
		 ...
		 function(state, initW, handlers) {
		 	arrayEach(handlers,
				function(x, i) {
					check(x, function(y) { return isWorldConfigOption(y) || isList(x) || types.isWorldConfig(x); },
...
PRIMITIVES['make-world-config'] =
    new PrimProc('make-world-config',
		 ...
		 function(startup, shutdown, handlers) {
		 	check(startup, isFunction, 'make-world-config', 'procedure', 1);
			check(shutdown, procArityContains(1), 'make-world-config', 'procedure (arity 1)', 2);
			arrayEach(handlers, function(x, i) { check(x, isFunction, 'make-world-config', 'handler', i+3); });
...
PRIMITIVES['make-effect-type'] =
	makeOptionPrimitive(
	    ...
	    [false],
	    true,
	    function(aState, name, parentType, fieldCnt, impl, guard) {
		check(name, isSymbol, 'make-effect-type', 'string', 1);
		check(parentType, function(x) { return x === false || types.isEffectType(x) },
		      'make-effect-type', 'effect type', 2);
		check(fieldCnt, isNatural, 'make-effect-type', 'exact non-negative integer', 3);
		check(impl, isFunction, 'make-effect-type', 'procedure', 4);
		check(guard, function(x) { return x === false || isFunction(x); }, 'make-effect-type', 'procedure or false', 5);

		var jsImpl = schemeProcToJs(aState, impl);
		var jsGuard = (guard ? schemeProcToJs(aState, guard) : false);
		var anEffectType = types.makeEffectType(name.toString(), parentType, fieldCnt, jsImpl, jsGuard);
...
PRIMITIVES['make-effect:do-nothing'] = new PrimProc('make-effect:do-nothing', 0, false, false, types.effectDoNothing);
PRIMITIVES['effect:do-nothing?'] = new PrimProc('effect:do-nothing?', 1, false, false, types.isEffectDoNothing);
...
PRIMITIVES['world-with-effects'] =
    new PrimProc('world-with-effects',
		 ...
		 false, false,
		 function(effects, w) {
		 	check(effects, isCompoundEffect, 'world-with-effects', 'compound effect', 1);
		 ...
		 function(aState, proc) {
			...
			aState.v = types.jsObject(proc.name + ' (cps)', function() {
				var args = helpers.map(arguments,
						       function(x) { return (types.isJsObject(x) ? x.obj : x); });
...
PRIMITIVES['get-named-js-object'] =
    new PrimProc('get-named-js-object',
		 1,
		 false, false,
		 function(name) {
		 	check(name, isString, 'get-named-js-object', 'string', 1);
...
PRIMITIVES['get-js-object'] =
    new PrimProc('get-js-object',
		 ...
		 function(root, firstSelector, selectors) {
		 	...
		 	check(root, isJsObject, 'get-js-object', 'js-object', 1);
    ...
    new PrimProc('js-set-field!',
		 ...
		 function(obj, field, val) {
		 	check(obj, function(x) { return isJsObject(x) && typeof(x.obj) == 'object'; },
			      'js-set-field!', 'javascript object', 1);
			check(field, isString, 'js-set-field!', 'string', 2);

			obj.obj[field.toString()] = (types.isJsObject(val) ? val.obj : val);
			return types.VOID;
		 });
...
PRIMITIVES['js-instanceof'] =
    new PrimProc('js-instanceof',
		 ...
		 false, false,
		 function(obj, type) {
		 	check(obj, types.isJsObject, 'js-instanceof', 'js-object', 1);
			check(type, isJsFunction, 'js-instanceof', 'javascript function', 2);
...
PRIMITIVES['js-call'] =
    new PrimProc('js-call',
		 ...
		 function(fun, parent, initArgs) {
		 	check(fun, isJsFunction, 'js-call', 'javascript function', 1);
			check(parent, function(x) { return (x === false ||
							    (types.isJsObject(x) && typeof(x.obj) == 'object')); },
			      'js-call', 'javascript object or false', 2);
			
			var args = helpers.map(initArgs, function(x) { return (types.isJsObject(x) ? x.obj : x); });
...
PRIMITIVES['make-js-hash'] =
    new CasePrimitive('make-js-hash',
	[new PrimProc('make-js-hash', 0, false, false, function() { return types.jsObject('hash', {}); }),
	 new PrimProc('make-js-hash',
		      1,
		      false, false,
		      function(bindings) {
			  checkListOf(bindings, function(x) { return isAssocList(x) && isString(x.first()); },
				      'make-js-hash', '(listof string X)', 1);

			  var ret = {};
			  while ( !bindings.isEmpty() ) {
			  	var key = bindings.first().first().toString();
				var val = bindings.first().rest().first();
				ret[key] = (types.isJsObject(val) ? val.obj : val);
