...
var car-first = function(name) {
	function (lst) {
		checkList(lst, name, 1);
		return lst.first();
	}
}
PRIMITIVES['first'] = new Primitive('first', 1, false, false, car-first('first'));
PRIMITIVES['car'] = new Primitives('car', 1, false, false, car-first('car'));


var cdr-rest = function(name) {
	function (lst) {
		checkList(lst, name, 1);
		return lst.rest();
	}
}
PRIMITIVES['rest'] = new Primitive('rest', 1, false, false, cdr-rest('rest'));
PRIMITIVES['cdr'] = new Primitive('cdr', 1, false, false, cdr-rest('cdr'));
...
var cadr-second = function(name) {
	function(lst) {
		checkList(lst, 'cadr', 1);
		lst.rest().first();
	}
}
PRIMITIVES['cadr'] = new Primitive('cadr', 1, false, false, cadr-second('cadr'));
PRIMITIVES['second'] = new Primitive('second', 1, false, false, cadr-second('second'));
...
var caddr-third = function(name) {
	function(lst) {
		checkList(lst, name, 1);
		lst.rest().rest().first();
	}
}
PRIMITIVES['caddr'] = new Primitive('caddr', 1, false, false, caddr-third('caddr'));
PRIMITIVES['third'] = new Primitive('third', 1, false, false, caddr-third('third'));
...
var cadddr-fourth = function(name) {
	function (lst) {
		checkList(lst, name, 1);
		lst.rest().rest().rest().first();
	}
}
PRIMITIVES['cadddr'] = new Primitive('cadddr', 1, false, false, cadddr-fourth('cadddr'));
PRIMITIVES['fourth'] = new Primitive('fourth', 1, false, false, cadddr-fourth('fourth'));
...
PRIMITIVES['list*'] =
    new Primitive('list*',
		  ...
		  function(items, otherItems) {
		  	...
		  	return append([list(otherItems), lastListItem]);
		  });
    ...
    new Primitive('map'
		  ...
		  function(f, lst, restlists) {
		  	...
		  	while (!arglists[0].isEmpty()) {
		  		...
		  		results = types.Cons.makeInstance(f(args), results);
		  	}
		  	return types.Cons.reverse(results);
		  });
...
PRIMITIVES['filter'] =
    new Primitive('filter',
		  ...
		  function(f, lst) {
			...
			while( !lst.isEmpty() ) {
				if ( f([lst.first()]) ) {
					results = types.Cons.makeInstance(lst.first(), results);
				}
				lst = lst.rest();
			}
			return types.Cons.reverse(results);
...
var list = function(items) {
	var ret = types.Empty.EMPTY;
	for (var i = items.length - 1; i >= 0; i--) {
		ret = types.Cons.makeInstance(items[i], ret);
	}
	return ret;
}
