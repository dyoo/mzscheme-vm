...
var isReal = helpers.isReal;
var isRational = helpers.isRational;
var isComplex = isNumber;
var isInteger = helpers.isInteger;
var isNatural = helpers.isNatural;
var isSymbol = helpers.isSymbol;
var isChar = helpers.isChar;
var isString = helpers.isString;
var isPair = helpers.isPair;
var isEmpty = helpers.isEmpty;
var isList = helpers.isList;
var isVector = helpers.isVector;
var isBox = helpers.isBox;
var isHash = helpers.isHash;
var isByteString = helpers.isByteString;
var isByte = helpers.isByte;
var isBoolean = helpers.isBoolean;
var isFunction = helpers.isFunction;
var isEqual = helpers.isEqual;
var isEqv = helpers.isEqv;
var isEq = helpers.isEq;

var arrayEach = helpers.arrayEach;
var check = helpers.check;
var checkList = helpers.checkList;
var checkListOf = helpers.checkListOf;
var checkAllSameLength = helpers.checkAllSameLength;
*/
...
PRIMITIVES['error'] =
    new PrimProc('error',
		 ...
		 function(arg1, args) {
			...
			if ( isSymbol(arg1) ) {
				if ( args.length === 0 ) {
					return types.schemeError(types.exnFail("error: " + arg1.val, false));
				}
				var formatStr = args.shift();
				check(formatStr, isString, 'error', 'string', 2);

				args.unshift(arg1);
				return types.schemeError(types.exnFail( format('~s: '+formatStr.toString(), args), false ));
			}
			else {
				...
				return types.schemeError(types.exnFail(msg, false));
			}
		 });
	...
	[new PrimProc('subbytes',
	 ...
	 new PrimProc('subbytes',
		      ...
		      function(bstr, theStart, theEnd) {
			   ...
			   var start = jsnums.toFixnum(theStart);
			   var end = jsnums.toFixnum(theEnd);
			   if (start > bstr.length() || end > bstr.length() || end < start) {
			  	// TODO Throw some sort of error here!
			   }
			   else {
...
var jsp = function(attribList) {
	...
	var node = jsworld.Jsworld.p(attribs);
...
var jsdiv = function(attribList) {
	checkListOf(attribList, function(x) { return isList(x) && length(x) == 2; },
		    'js-div', 'list of (list of X Y)', 1);
	var attribs = assocListToHash(attribList);
	var node = jsworld.Jsworld.div(attribs);
...
/*
var jsButtonBang = funciton(worldUpdateF, effectF, attribList) {
	var attribs = assocListToHash(attribList);
	var wrappedF = 
*/
