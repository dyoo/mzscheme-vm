...
PRIMITIVES['make-world-config'] =
    new PrimProc('make-world-config',
		 2,
		 true, false,
		 function(startup, shutdown, args) {
			...
			if ( !funArityContains(args.length)(startup) ) {
			...
			return types.worldConfig(startup, shutdown, args);
		 });
...
PRIMITIVES['coerce-scheme->js'] =
    new PrimProc('coerce-scheme->js',
		 1,
		 false, false,
		 function(x) {
		 	check(x, function(y) { return ( isReal(y) ||
							...
							isBoolean(y) ||
							isFunction(y)); },
			      'coerce-scheme->js', 'coerceable scheme value', 1);

		 	if ( isReal(x) ) {
				return jsnums.toFixnum(x);
			}
			else if ( isString(x) ) {
				return x.toString();
			}
...
PRIMITIVES['coerce-js->scheme'] =
    new PrimProc('coerce-js->scheme',
		 1,
		 false, false,
		 function(x) {
		 	check(x, function(y) { return ( typeof(y) == 'number' ||
							typeof(y) == 'string' ||
							typeof(y) == 'boolean' ||
							typeof(y) == 'function' ); },
			      'coerce-js->scheme', 'coerceable javascript value', 1);
		 	...
		 	if ( typeof(x) === 'number' ) {
				...
				return new PrimProc('', 0, true, false, function(args) { return x.apply(null, args); });
			}
		 });
