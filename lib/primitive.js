...
var quicksort = function(functionName) {
	return function(initList, comp) {
		...
		check(comp, isFunction, functionName, 'function', 2);
	
		var quicksortHelp(lst) {
			if ( lst.isEmpty() ) {
				return k(runtime.EMPTY);
			}
	
			return CALL(filter, [comp, lst.rest()],
				function(half1) {
					return CALL(quicksortHelp, [half1],
						function(sorted1) {
							return CALL(filter, [function(x, y) {return !comp(x, y);}, lst.rest()],
								function(half2) {
									return CALL(quicksortHelp, [half2],
										function(sorted2) {
...
var isString = function(x) {
	...
	return x !== null && x !== undefined && x instanceof types.Str;
}
...
var isVector = function(x) {
	return (x !== null && x !== undefined && x instanceof types.Vector);
}
...
PRIMITIVES['void'] =
    new Primitive('void', 0, true, false,
		  function(args) {
		  	return types.VOID;
		  });
...
PRIMITIVES['procedure?'] = new Primitive('procedure?', 1, false, false, isFunction);
...
PRIMITIVES['null?'] = new Primitive('null?', 1, false, false, isEmpty);
...
PRIMITIVES['vector?'] = new Primitive('vector?', 1, false, false, isVector);
PRIMITIVES['struct?'] = new Primitive('struct?', 1, false, false, types.isStruct);
...
PRIMITIVES['eq?'] = new Primitive('eq?', 2, false, false, isEq);
PRIMITIVES['eqv?'] = new Primitive('eqv?', 2, false, false, isEqv);
PRIMITIVES['equal?'] = new Primitive('equal?', 2, false, false, isEqual);
...
PRIMITIVES['filter'] = new Primitive('filter', 2, false, false, filter);
...
PRIMITIVES['quicksort'] = new Primitive('quicksort', 2, false, false, quicksort('quicksort'));
PRIMITIVES['sort'] = new Primitive('sort', 2, false, false, quicksort('sort'));



PRIMITIVES['argmax'] =
    new Primitive('argmax',
		  2,
		  false, false,
		  function(f, initList) {
		  	check(f, isFunction, 'argmax', 'function', 1);
			check(initList, isCons, 'argmax', 'non-empty list', 2);
...
PRIMITIVES['argmin'] =
    new Primitive('argmin',
		  2,
		  false, false,
		  function(f, initList) {
		  	check(f, isFunction, 'argmin', 'function', 1);
			check(initList, isCons, 'argmin', 'non-empty list', 2);
...
PRIMITIVES['replicate'] =
    new Primitive('replicate',
		  ...
		  function(n, str) {
			...
			for (var i = 0; jsnums.issThan(i, n); i++) {
				ret += primStr;
			}
			return types.string(ret);
		  });
...
PRIMITIVES['format'] = new Primitive('format', 1, true, false, format('format'));
    new Primitive('format', 1, true, false,
...
PRIMITIVES['printf'] =
    new Primitive('printf', 1, true, false,
		  function(formatStr, args) {
