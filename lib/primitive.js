	...
	return new types.ValuesWrapper(
		[aStructType,
		 ...
		 (new StructAccessorProc(name,
					 ...
					 false,
					 function(x, i) {
						check(x, aStructType.predicate, name+'-ref', 'struct:'+name, 1, [x, i]);
						check(i, isNatural, name+'-ref', 'non-negative exact integer', 2, [x, i]);
	...
	makeOptionPrimitive(
	    ...
	    function(accessor, fieldPos, fieldName) {
	    	check(accessor, function(x) { return x instanceof StructAccessorProc && x.numParams > 1; },
		      'make-struct-field-accessor', 'accessor procedure that requires a field index', 1, arguments);
		check(fieldPos, isNatural, 'make-struct-field-accessor', 'exact non-negative integer', 2, arguments);
		check(fieldName, function(x) { return x === false || isSymbol(x); },
		      'make-struct-field-accessor', 'symbol or #f', 3, arguments);
	...
	makeOptionPrimitive(
	    ...
	    function(mutator, fieldPos, fieldName) {
	    	check(mutator, function(x) { return x instanceof StructMutatorProc && x.numParams > 1; },
		      'make-struct-field-mutator', 'mutator procedure that requires a field index', 1, arguments);
		check(fieldPos, isNatural, 'make-struct-field-mutator', 'exact non-negative integer', 2, arguments);
		check(fieldName, function(x) { return x === false || isSymbol(x); },
		      'make-struct-field-mutator', 'symbol or #f', 3, arguments);
	    	var fixnumPos = jsnums.toFixnum(fieldPos);
...
PRIMITIVES['make-effect-type'] =
	makeOptionPrimitive(
	    ...
	    [false],
	    true,
	    function(userArgs, aState, name, parentType, fieldCnt, impl, guard) {
		...
		check(guard, function(x) { return x === false || isFunction(x); }, 'make-effect-type', 'procedure or false', 5, userArgs);
    ...
    new PrimProc('get-js-object',
		 ...
		 function(root, firstSelector, selectors) {
			...
			var allArgs = [root].concat(selectors);
		 	check(root, isJsObject, 'get-js-object', 'js-object', 1, allArgs);
			arrayEach(selectors, function(x, i) { check(x, isString, 'get-js-object', 'string', i+2, allArgs); });
...
PRIMITIVES['js-instanceof'] =
    new PrimProc('js-instanceof',
		 ...
		 false, false,
		 function(obj, type) {
		 	check(obj, types.isJsObject, 'js-instanceof', 'js-object', 1, arguments);
