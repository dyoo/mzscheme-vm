...
PRIMITIVES['apply'] =
    new PrimProc('apply',
		 ...
		 function(f, firstArg, args) {
			...
			args.concat(schemeListToArray(lastArg));
...
PRIMITIVES['values'] =
    new PrimProc('values',
		 ...
		 function(args) {
		 	...
		 	return types.ValuesWrapper(args);
		 });
...
PRIMITIVES['call-with-values'] =
    new PrimProc('call-with-values',
		 ...
		 false, false,
		 function(g, r) {
		 	check(g, isFunction, 'call-with-values', 'function', 1);
			...
			return CALL(g, [],
				function(res) {
					...
					else if (res instanceof types.ValuesWrapper) {
						return CALL(r, res.elts, id);
					}
					else {
						return CALL(r, res, id);
					}
				});
		 });
*/
...
PRIMITIVES['identity'] =
    new PrimProc('identity', 1, false, false, function(x) {return x;});
...
PRIMITIVES['filter'] = new PrimProc('filter', 2, false, false, filter);
...
PRIMITIVES['make-hash'] = new PrimProc('make-hash', 0, false, false, types.EqualHashTable);

PRIMITIVES['make-hasheq'] = new PrimProc('make-hasheq', 0, false, false, types.EqHashTable);
...
PRIMITIVES['implode'] =
    new PrimProc('implode',
		 ...
		 function(lst) {
			...
			return types.string(schemeListToArray(lst));
		 });
...
PRIMITIVES['eof'] = new PrimitiveConst('eof', types.EOF);
PRIMITIVES['e'] = new PrimitiveConst('e', jsnums.e);
PRIMITIVES['empty'] = new PrimitiveConst('empty', types.EMPTY);
PRIMITIVES['false'] = new PrimitiveConst('false', false);
PRIMITIVES['true'] = new PrimitiveConst('true', true);
PRIMITIVES['pi'] = new PrimitiveConst('pi', jsnums.pi);
PRIMITIVES['null'] = new PrimitiveConst('null', types.EMPTY);
...
primitive.PrimitiveConst = PrimitiveConst;
