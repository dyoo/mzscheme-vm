...
var makeCaller = function(aState) {
	...
	return function(operator, operands, k) {
		interpret.call(aState, operator, operands, k, onFail);
	}
};
...
PRIMITIVES['make-struct-field-accessor'] =
	makeOptionPrimitive(
		...
		return new StructAccessorProc(accessor.type, procName, 1, false, false,
					      function(x) {
						  check(x, accessor.type.predicate, procName, 'struct:'+types.name, 1);
...
PRIMITIVES['make-struct-field-mutator'] =
	makeOptionPrimitive(
		...
		return new StructMutatorProc(mutator.type, procName, 2, false, false,
					     function(x, v) {
						 check(x, accessor.type.predicate, procName, 'struct:'+types.name, 1, arguments);
