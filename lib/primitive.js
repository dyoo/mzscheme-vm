...
var checkList = function(x, functionName, position) {
	if (! isList(x)) {
		sys.print("Non-list given as argment " + position + " in " + functionName + "\n");
...
PRIMITIVES['for-each'] =
    new Primitive('for-each', 
		  2, 
		  true, true,
		  function(state, f, firstList, arglists) {
		  	arglists.unshift(firstList);
		  ...
		  function(f, lst, arglists) {
			...
			var mapHelp = function(f, args, acc) {
				...
				var result = CALL(f, argsFirst,
					function(result) {
					        sys.print("after application, " + result);
						return mapHelp(f, argsRest, types.Cons.makeInstance(result, acc));
					});
				sys.print("call says: " + sys.inspect(result) + "\n");
				return result;
			}
