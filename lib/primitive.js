...
var schemeProcToJs = function(aState, schemeProc) {
	var caller = function(operator, operands, k) {
		interpret.call(aState, operator, operands, k, function(e) { throw e; }, 'schemeProcToJs: ' + schemeProc.name);
	}
	return function(k) {
		var args = [];
		 ...
		 function(x) {
			...
			var returnVal;
		 	if ( isReal(x) ) {
				...
				returnVal = helpers.map(function(y) { return (isJsObject(y) ? y.obj : y); },
							x.elts);
			}
    ...
    new PrimProc('prim-js->scheme',
		 ...
		 function(x) {
		 	...
		 	if ( typeof(x.obj) === 'number' ) {
				...
				return types.vector( helpers.map(helpers.wrapJsObject, x.obj) );
			}
		 });
...
PRIMITIVES['procedure->cps-js-fun'] =
    new PrimProc('procedure->cps-js-fun',
		 ...
		 function(proc) {
			...
			return PAUSE(function(caller, onSuccess, onFail) {
				onSuccess(types.jsObject(proc.name + ' (void)', function() {
						var args = helpers.map(helpers.wrapJsObject, arguments);
						var k = (args.length == 0 ? function() {} : args.shift().obj);

						caller(proc, args, k, onFail, 'proc->void-js: ' + proc.name);
					  }));
				});
		 });
    ...
    new PrimProc('procedure->void-js-fun',
		 ...
		 function(proc) {
			...
			return PAUSE(function(caller, onSuccess, onFail) {
				onSuccess(types.jsObject(proc.name + ' (void)', function() {
						var args = helpers.map(helpers.wrapJsObject, arguments);
						caller(proc, args, function() {}, onFail, 'proc->void-js: ' + proc.name);
					  }));
				});
		 });
