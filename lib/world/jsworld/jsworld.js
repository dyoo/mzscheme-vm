	...
	*/
	...
	return function() {
	    var wrappedTilt;
		    ...
		    preventDefault(e);
		    stopPropagation(e);
                    beta = e.acceleration.x * 2;
                    gamma = e.acceleration.y * 2;
                };
	        return {
		    onRegister: function(top) { attachEvent(window, 'devicemotion', wrappedTilt); 
                                                tickId = setInterval(update, delay); },
		    onUnregister: function(top) { detachEvent(window, 'devicemotion', wrappedTilt);
                                                  clearInterval(tickId); }
	        };
            } else {
                wrappedTilt = function(e) {
