...
(function() {
	...
	var changeWorldHelp = function(i) {
		if (i >= worldListeners.length) {
			k();
		}
		try {
			worldListeners[i](world, originalWorld,
					  function() { changeWorldHelp(i+1); });
		} catch (e) {
			world = originalWorld;
			throw e;
		}
...
*/
	...
	var foldHelp(i, acc) {
		...
		f(a[i], acc, function(res) { foldHelp(i+1, res); });
	}
	foldHelp(0, x);
    }
    Jsworld.fold = fold;
	...
	var removefHelp(i) {
		...
		f(array[i],
		  function(res) {
			if (res) {
				k( array.slice(0, i).concat(array.slice(i+1)) );
			}
			else {
				removefHelp(i+1);
			}
...
//	}
	...
	change_world(function(w, k) { k(init_world); }, doNothing);
	...
	return function(k) {
	    ...
	    k( new StopWhenHandler(test, receiver) );
	};
    }
