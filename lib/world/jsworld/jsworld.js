...
//	clear_running_state();
	...
	*/
	...
	if (node.removeEventListener) {
	    // Mozilla
	    setTimeout(function() { node.removeEventListener(eventName, fn, false); }, 0);
	} else {
	    // IE
	    setTimeout(function() { node.detachEvent('on' + eventName, fn, false); }, 0);
	}
    }
    ...
    var add_ev = function(node, event, f) {
    ...
    var add_ev_after = function(node, eventName, f) {
	var eventHandler = function(e) {
		setTimeout(function() { change_world(function(w, k) { f(w, e, k); },
						     doNothing); },
			   0);
	};
	attachEvent(node, eventName, eventHandler);
	eventDetachers.push(function() { detachEvent(node, eventName, eventHandler); });
    }
...
//	    });
	...
	return stopClickPropagation(copy_attribs(n, attribs));
    };
