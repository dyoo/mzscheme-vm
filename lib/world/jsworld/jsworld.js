...
//	var result = document.createTextNode(s);
    ...
    Page.prototype.add = function(elt, positionTop, positionLeft) {
    ...
    Page.prototype.toTree = function() {
	var aDiv = div();
	for (var i = 0 ; i < this.elts.length; i++) {
	    var elt = elts[i].elt;
	    elt.style.position = 'absolute';
	    elt.style.top = elts[i].positionTop;
	    elt.style.left = elts[i].positionLeft;
	    aDiv.appendChild(elt);
	};
	copy_attribs(aDiv, this.attribs)
	return node_to_tree(aDiv);
    };


    Jsworld.isPage = function(x) {
	return x instanceof Page;
    };

    Jsworld.emptyPage = function(attribs) {
	return new Page([], attribs);
    };
	...
	return page.add(elt, positionTop, positionLeft);
    };
