	...
	*/
	...
	return function() {
	    var wrappedTilt;
		    ...
		    preventDefault(e);
		    stopPropagation(e);
		    change_world(function(w, k) { tilt(w, e.beta, e.gamma, k); }, doNothing);
                };
	        return {
		    onRegister: function(top) { attachEvent(top, 'deviceorientation', wrappedTilt); },
		    onUnregister: function(top) { detachEvent(top, 'deviceorientation', wrappedTilt); }
	        };
            } else if (window.DeviceMotionEvent) {
                wrappedTilt = function(e) {
		    preventDefault(e);
		    stopPropagation(e);
		    change_world(function(w, k) { tilt(w, 
                                                       e.acceleration.x * 2,
                                                       e.acceleration.y * 2,
                                                       k); }, 
                                 doNothing);
	        ...
	        return {
		    onRegister: function(top) { attachEvent(top, 'devicemotion', wrappedTilt); },
		    onUnregister: function(top) { detachEvent(top, 'devicemotion', wrappedTilt); }
	        };
            } else {
                wrappedTilt = function(e) {
		    preventDefault(e);
	        ...
	        return {
		    onRegister: function(top) { attachEvent(top, 'MozOrientation', wrappedTilt); },
		    onUnregister: function(top) { detachEvent(top, 'MozOrientation', wrappedTilt); }
	        };
            }
	}
    }
