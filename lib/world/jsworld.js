...
//			     	console.log('    shutting down a config');
	...
	return types.jsObject('function', function() {
		...
		function(w, k) {
			...
			caller(handler, args, function(w) { k(unwrapWorldEffects(w)); });
		},
		function() {});
	});
    };
	...
	if ( _js.has_effects(w) ) {
		var unwrappedEffects =
			helpers.map(w.getEffects(),
				    function(e) {
					if ( types.isEffect(e) ) {
						return types.makeJsworldEffect(function(k2) {
								caller(types.effectThunk(e), [], k2);
							});
					}
					else {
						return e;
		...
		var returnVal = new _js.with_multiple_effects(w.getWorld(), unwrappedEffects);
		return returnVal;
	}
	else {
		return w;
...
//		console.log('Got an error, the error was:');
		...
		if ( types.isSchemeError(e) ) {
			...
			restarter( types.schemeError(types.incompleteExn(types.exnFail, e, [])) );
		}
		else if (e instanceof Error) {
			restarter( types.schemeError(types.incompleteExn(types.exnFail, e.message, [])) );
		}
		else {
