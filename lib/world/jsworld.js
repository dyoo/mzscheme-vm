...
(function() {
    ...
    var expandWrappedHandler = function(wrappedHandler) {
	    return function() {
		    var stimulusArgs = [];
		    for (var i = 0; i < arguments.length; i++) {
			    stimulusArgs.push(arguments[i]);
		    }

		    Jsworld.updateWorld(
			function(w, k) {
			    var args = [w].concat(stimulusArgs);
			    caller(wrappedHandler.handler, args, k);
			},
			function() {});
	    }
    };
...
//    }
	...
	for(var i = 0; i < handlers.length; i++) {
		...
		config = handlers[i](config);
		    ...
		    handlers[i].startupArgs = helpers.map(handlers[i].startupArgs, expandWrappedHandler);
