...
(function() {
	...
	var BigBang = function(caller, onSuccess, onFail, toplevelNode) {
		...
		var schemeChangeWorld =
			new types.PrimProc('change-world', 1, false, false,
				function(updater) {
					...
					return PAUSE(function(newCaller, onSuccess, onFail) {
						that.changeWorld(function(w, k) { newCaller(updater,
											    [w], k,
											    function(e) { throw e; },
											    'change-world'); },
								 function() { onSuccess(types.VOID, 'restarting (change-world)'); });
					});
				});
...
//					k([aBigBang.toplevelNode, nodeToTree( types.toDomNode(aScene) )]);
	...
	var bigBang = function(initWorld, toplevelNode, handlers, caller, onSuccess, onFail, controller) {
		var newBigBang = new BigBang(caller, onSuccess, onFail, toplevelNode);
