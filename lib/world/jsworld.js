...
//		    throw e;
	...
	if (config.lookup('onTilt')) {
	    var wrappedTilt = function(w, beta, gamma, k) {
		caller(config.lookup('onTilt'), [w, jsnums.makeFloat(beta), jsnums.makeFloat(gamma)], k);
	    }
	    wrappedHandlers.push(_js.on_key(wrappedTilt));
	    toplevelNode.focus();
	}
