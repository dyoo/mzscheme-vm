...
(function() {
	...
	var BigBang = function(aState, caller) {
		...
		this.toplevelNode = aState.getToplevelNodeHook()();
		this.toplevelNode.tabIndex = 0;

		this.state = aState;
		this.hasEnded = false;
		this.setOnFail(aState.onFail);
		this.setOnSuccess(aState.onSuccess);

		this.setCaller(caller);

		var that = this;
		this.state.onFail = function(e) { that.handleError(e); };
		this.state.onSuccess = function(v) {
			that.shutdown(v, that.onSuccess);
		};
		...
		var schemeChangeWorld =
			new types.PrimProc('change-world', 1, false, false,
				function(updater) {
					...
					return PAUSE(function(newCaller, newOnSuccess, newOnFail) {
						that.changeWorld(function(w, k) { newCaller(updater,
											    [w], k,
											    function(e) { throw e; },
											    'change-world'); },
								 function() { newOnSuccess(types.VOID, 'restarting (change-world)'); });
					});
				});
		this.configStartupArg = types.makeBigBangInfo( schemeChangeWorld, helpers.wrapJsValue(this.toplevelNode) );
	};
	...
	BigBang.prototype.setOnSuccess = function(k) {
		var that = this;
		this.onSuccess = function(v) {
			if (that.hasEnded) {
				throw types.internalError('A single big-bang cannot end twice! (this succeeded)', false);
			}
			that.hasEnded = true;
			k(v);
		};
	...
	BigBang.prototype.setOnFail = function(k) {
		var that = this;
		this.onFail = function(e) {
			if (that.hasEnded) {
				throw types.internalError('A single big-bang cannot end twice! (this failed)', false);
			}
			that.hasEnded = true;
			k(e);
		};
	...
	BigBang.prototype.shutdown = function(v, restarter) {
		this.world = new InitialWorld();
		this.worldListeners = [];
	...
	BigBang.prototype.shutdownUserConfigs = function(k) {
		...
		helpers.forEachK(this.userConfigs,
				 function(aConfig, k2) {
				 	if ( aConfig.shutdownArg !== undefined ) {
						that.caller(aConfig.shutdown,
							    [aConfig.shutdownArg],
							    function(res) {
							    	aConfig.restartArg = res;
								aConfig.shutdownArg = undefined;
								k2();
							    },
							    function(e) {
								// if we catch an error, we still want to shut down other configs
								// so catch the error and keep moving
							    	aConfig.shutdownArg = undefined;
							    	error = error || e;
								k2();
							    },
				 ...
				 function(e) { throw e; },
				 function() {
					// if we got an error, we want to throw it, but to prevent being
					// caught in the previous error trappers, we just return and throw it later
				 	if (error) { return; }
					k();
				 });

		// if there was an error, throw it now since we're out
		// of the scope of the error trappers for the shutdown calls
		if (error) {
			throw error;
		}
	};
	...
	var deepUnwrapDomTree = function(x) {
		if ( types.isJsValue(x) ) {
			return deepUnwrapDomTree(x.val);
		}
		else if ( isPair(x) ) {
			var ret = [];
			while ( !x.isEmpty() ) {
	...
	BigBang.prototype.handleError = function(e) {
		...
		try {
			...
			this.shutdown(wrapError(e), this.onFail);
		} catch(e2) {
			this.onFail( wrapError(e) );
		}
	};
...
//		}
	...
	var doRedraw = function(aBigBang, w, redrawFun, redrawCssFun, k) {
		...
		var redrawPage = function(newPage, newCss, k2) {
		...
		maintainingSelection(
		    function(k2) {
			redrawFun(w, function(newPage) {
				redrawCssFun(w, function(newCss) {
					redrawPage(newPage, newCss, k2);
				});
			});
		    }, k);
	};
	...
	StopWhenConfig.prototype.getHandler = function(aBigBang) {
		...
		return function(w, k) {
			aBigBang.caller(that.test,
					...
					function(stop) {
						if (stop) {
							aBigBang.shutdown(w, aBigBang.onSuccess);
						} else {
							k();
						}
					},
			...
			aBigBang.caller(that.draw, [w],
				...
				if ( world.Kernel.isImage(aScene) ) {
					...
					if ( !reusableCanvas ) {
						...
						// Node: the canvas object may itself manage objects,
...
//					k([aBigBang.toplevelNode, nodeToTree( types.toDomNode(aScene) )]);
	...
	var bigBang = function(initWorld, handlers, aState, caller, controller) {
		var newBigBang = new BigBang(aState, caller);
	...
	var getBaseNode = function(node) {
		if ( types.isJsValue(node) ) {
			return getBaseNode(node.val);
		}
