...
(function() {
    ...
    var expandCoercedHandler = function(coercedHandler) {
	    return function() {
		    var stimulusArgs = [];
			...
			function(w, k) {
			    ...
			    caller(coercedHandler.handler, args, k);
			},
			function() {});
	    }
    };
...
//    }
	...
	for(var i = 0; i < handlers.length; i++) {
		...
		config = handlers[i](config);
		    ...
		    handlers[i].startupArgs = helpers.map(handlers[i].startupArgs, expandCoercedHandler);
