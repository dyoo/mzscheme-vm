...
(function() {
	...
	var BigBang = function(caller, restarter, toplevelNode) {
		...
		var schemeChangeWorld =
			new types.PrimProc('change-world', 1, false, true,
				function(aState, updater) {
					helpers.check(updater, helpers.procArityContains(1),
						      'change-world', 'procedure (arity 1)', 1);
					that.changeWorld(function(w, k) { that.caller(updater, [w], k, function(e) { throw e; }); },
							 function() { aState.v = types.VOID; });
				});
	...
	BigBang.prototype.setCaller = function(c) {
		var that = this;
		this.caller = function(op, args, k, onFail) {
			if ( !onFail ) {
				onFail = function(e) { that.handleError(e); };
			}
			return c(op, args, k, onFail);
		};
	};
	...
	BigBang.prototype.changeWorld = function(updater, k) {
		...
		var changeWorldHelp2 = function() {
			 helpers.forEachK(that.worldListeners,
					  function(listener, k2) { listener(that.world, originalWorld, k2); },
					  function(e) { throw e; },
					  k);
		};
...
//		var that = this;
//		this.shutdownUserConfigs(function() {
//			that.world = new InitialWorld();
//			that.worldListeners = [];
//			that.rendertimeDetachEvents();
//			detachEvent(that.toplevelNode, 'click', that.absorber);
//			that.restarter(w);
//		});
	};
...
//	var checkWellFormedDomSexp = function(x, originalTree) {
//		var fail = function(formatStr, formatArgs) {
//			helpers.raise(types.incompleteExn(
//					types.exnFailContract,
//					helpers.format(formatStr, formatArgs),
...
//			var node = getBaseNode(x[0]);
//
//			if ( !isNode(node) ) {
//				fail('on-draw: found illegal element ~s in dom-tree; entire tree was: ~s',
//				     [node, originalTree]);
//			}
//
//			if (node.nodeType == Node.TEXT_NODE && x.length > 1) {
//				fail('on-draw: the text node ~s cannot have children. Entire tree was: ~s',
//				     [node, originalTree]);
//			}
//
//			for (var i = 1; i < x.length; i++) {
//				checkWellFormedDomSexp(x[i], originalTree);
//			}
//			var i = 2;
//		} else {
//			var formatStr = "on-draw: expected a dom-s-expression, but received ~s inside ~s";
//			fail("on-draw: expected a dom-s-expression, but received ~s inside ~s",
//			     [x, originalTree]);
//		}
//	};
...
//		}
	...
	var doRedraw = function(aBigBang, w, oldW, redrawFun, redrawCssFun, k) {
		aBigBang.rendertimeDetachEvents();
		if (oldW instanceof InitialWorld) {
			// Simple path
			redrawFun(w,
			    function(drawn) {
				var t = sexp2tree(drawn, aBigBang);
				var ns = nodes(t);
			...
			function(k2) {
			    ...
			    /*
			    var oldPage = redrawFun(oldW);
 			    var newPage = redrawFun(w);		
 			    var oldCss = redrawCssFun(oldW);
	...
	var OnDrawPageConfig = DrawConfig.extend({
		...
		getHandler: function(aBigBang) {
		    ...
		    return function(w, oldW, k) {
			doRedraw(aBigBang, w, oldW, wrappedRedraw, wrappedRedrawCss, k);
		    };
		}
	});
	...
	var bigBang = function(initWorld, toplevelNode, handlers, caller, restarter, controller) {
		...
		controller.breaker = function() {
			newBigBang.handleError(types.schemeError(
...
//		return (node instanceof NodeWithEvents ? node.node : node);
	};
