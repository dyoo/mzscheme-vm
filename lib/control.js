...
var installModuleVariable = function(aState, newPrefix, top) {
    var resolvedModuleName = resolveModulePathIndex(top['modidx']);
...
// resolveModulePathIndex -> resolved-module-path
var resolveModulePathIndex = function(modulePathIndex) {
...
RequireControl.prototype.invoke = function(aState) {
    var that = this;
    var resolvedModuleName = resolveModulePathIndex(this.name);
	...
	if (aState.invokedModules[resolvedModuleName]) {
		...
		invokeModuleAndRestart(aState, moduleRecord, onSuccess, onFail);
	    } else {
