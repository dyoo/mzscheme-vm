...
//     newPrefix.addSlot(new types.NamedSlot(sym+''));
};
...
var resolveModulePathIndex = function(modulePathIndex, aState) {
    ...
    // FIXME: add special case for
    // modbeg.rkt <- "pre-base.rkt" <- "namespace.rkt" <- "private/base.rkt"
    //            <- racket/base
    // because we're reusing print-values from racket/base's
    // #%module-begin.
...
RequireControl.prototype.invoke = function(aState) {
    ...
    console.log("trying to require " + resolvedModuleName);
	...
	if (aState.invokedModules[resolvedModuleName] || 
	    ...
	    console.log("already required or built in");
	    // Already invoked.
	    onSuccess(types.VOID);
	} else {
	    console.log("module needs to be invoked");
	    console.log(resolvedModuleName);
	    // KLUDGE
	    if (typeof(COLLECTIONS) !== 'undefined' &&
		COLLECTIONS[resolvedModuleName]) {
		console.log("invoking module");
		var moduleRecord = COLLECTIONS[resolvedModuleName];
		invokeModuleAndRestart(aState, 
				       moduleRecord, 
				       function() {
					   console.log("successful invocation");
					   onSuccess() 
				       }, 
				       onFail);
	    } else {
...
var isHardcodedModule = function(resolvedModuleName) {
    return resolvedModuleName === types.symbol("moby/moby-lang");
};
...
var invokeModuleAndRestart = function(aState, moduleRecord, onSuccess, onFail) {
    var modulePrefix;
    var newOnSuccess = function() {
	var providedValues = {};
	for (var i = 0; i < moduleRecord.provides.length; i++) {
	    // FIXME: we should record the module's prefix and
	    // look things up there.  This is currently broken.
	    var providedName = moduleRecord.provides[i];
	    var globalBucket = aState.globals[providedName]
	    if (! globalBucket) {
		onFail(types.internalError(
...
//	    providedValues[providedName] = modulePrefix.lookup(providedName);
	}
    ...
    modulePrefix = aState.vstack[aState.vstack.length-1];
