...
// 	    "unable to install toplevel element " + top.sym,
// 	    state.captureCurrentContinuationMarks(aState)); 
    }
};
...
RequireControl.prototype.invoke = function(state) {
    var that = this;
    console.log("I'm trying to require " + that.name);
    var onPause = function(onRestart, onCall) {
	if (state.invokedModules[that.name]) {
		...
		console.log("I'm supposed to evaluate " + that.name);
	    } else {
		console.log("Ugh, no way to invoke that module");
	    }
	}
    };
