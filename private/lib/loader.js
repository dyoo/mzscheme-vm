...
var loadMod = function(state, modCode) {
    return new control.ModControl(loadPrefix(modCode['prefix']),
				  loadRequires(modCode['requires']),
				  loadCodes(state, modCode['body']));
};

var loadRequires = function(requires) {
    var loadedRequires = [];
    for (var i = 0; i < requires.length; i++) {
	var aRequire = requires[i];
	var phase = aRequire[0];
	var paths = aRequire.slice(1);
	if (phase === 0) {
	    for (var j = 0; j < paths.length; j++) {
		loadedRequires.push(loadModulePathIndex(paths[j]));
	    }
	} else {
	    // At the moment, ignore other phases.
	}
    }
    return loadedRequires;
};


var loadModulePathIndex = function(modulePathIndex) {
    // FIXME
    // This should take the structure and translate the module path
    // index to something more digestible.
    return modulePathIndex;
};
