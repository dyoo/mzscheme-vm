...
(provide/contract [compile-top (compilation-top? . -> . instruction?)]

                  ; For debugging purposes: do not use this
                  ; in production code.
                  [reset-label-counter! (-> any)])

;; Global parameters.

;; indirect-label-hash: symbol -> symbol
;; Given an indirect name, return the label for the text segment.
(define indirect-label-hash (make-parameter (make-hasheq)))
...
;; fresh-label: symbol -> symbol
;; Generates a fresh symbolic label.
(define label-counter 0)
(define (fresh-label prefix)
  (set! label-counter (add1 label-counter))
  (string->symbol (format "~a~a" prefix label-counter)))

(define (reset-label-counter!)
  (set! label-counter 0))


;; indirect-label: symbol -> symbol
;; Given an indirect's name, produces the label of the text segment,
;; generating a fresh name if necessary.
(define (indirect-label name)
  (hash-ref (indirect-label-hash) 
            name 
            (lambda () 
              (fresh-label "indirect"))))





;; compile-top: top -> instruction
(define (compile-top a-top)
  (parameterize ([seen-indirects (make-hasheq)]
                 [indirect-label-hash (make-hasheq)])
    (match a-top
      [(struct compilation-top (prefix code))
       (let* ([after-indirects (fresh-label 'after-indirects)]
              [compiled-code (compile-at-form-position code 'return)]
         ...
         (instruction-append (make-goto after-indirects)
                             (emit-indirects)
                             after-indirects
...
(define (emit-indirects)
  ...
  (let ([ht (seen-indirects)])
    (apply instruction-append 
           (for/list ([id+lam (in-hash-pairs ht)])
             (let ([lam-label (indirect-label (car id+lam))]
                   [lam-code (compile-lam (cdr id+lam) 'next)])
               (instruction-append lam-label
                                   lam-code))))))
...
;; compile-prefix: prefix -> instruction
(define (compile-prefix a-prefix)  
  ...
  (define (compile-module-variable a-module-variable)
    (match a-module-variable
      [(struct module-variable (modidx sym pos phase))
       (make-ModuleVariable sym
                            (compile-module-path-index modidx)
                            pos
                            phase)]))
  ...
  (match a-prefix
    [(struct prefix (toplevels stxs))
     ;; FIXME: handle stxs?
     (make-install-prefix (compile-toplevels toplevels))]))
...
(define (compile-form a-form linkage)
  (match a-form
    [(? def-values?)
     (compile-def-values a-form linkage)]
    [(? req?)
     (compile-req a-form linkage)]
    [(? seq?)
     (compile-seq a-form linkage)]
    [(? splice?)
     (compile-splice a-form linkage)]
    [(? mod?)
     (compile-mod a-form linkage)]
    [(? expr?)
     (compile-expr a-form linkage)]))
...
(define (compile-at-form-position x linkage)
  (match x
    [(? form?)
     (compile-form x linkage)]
    [(? indirect? x)
     (compile-indirect x linkage)]
    [else
     (compile-constant x linkage)]))
...
;; compile-seq: seq -> instruction
(define (compile-seq a-seq linkage)
  (match a-seq
    [(struct seq (forms))
     (make-no-op)]))
#;(make-ht 'seq 
